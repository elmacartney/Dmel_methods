...
```{r}
data_path = './data/locomotion/'
data_files = list.files('./data/locomotion/', recursive = T)
head(data_files)
...
id_index = grep('.*Subject Identification\\"\,\\"\s([A-Z0-9]{7}_[A-B]_[0-9]{6}_[0-9])\\"', head_temp)
run_id = gsub('.*Subject Identification\\"\,\\\"\s([A-Z0-9]{7}_[A-B]_[0-9]{6}_[0-9])\\\"', '\\1', head_temp[id_index])
...
for (file_i in data_files) {
  ...
  if(is.null(dat_df)) dat_df = dat_temp_df else dat_df = rbind(dat_df, dat_temp_df)
}
...
data_path = './data/ymaze/'
data_files = list.files('./data/ymaze/', recursive = T)
head(data_files)
